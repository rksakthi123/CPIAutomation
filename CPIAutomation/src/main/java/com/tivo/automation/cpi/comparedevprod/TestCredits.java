...
public class TestCredits {
	...
	public void test() throws JsonParseException, JsonMappingException, org.apache.wink.json4j.JSONException, IOException, ClassNotFoundException, JSONException, SQLException, ParseException {
		...
		for(String client:clients) {
			inputDetails.setFilePath("/Users/skaliyaperumal/Projects/CPI/13 Mar 2018/CRAWL/all_data_20180313T090030/"+client+"_cast.data");
		...
		String testingAttributes=getTestingAttribute(nonTestingAttribute,listProdData);
		...
		mappingDetails.setPrimaryAttribute("credit_id");
	...
	//	List<WebVideoProgramContentAvailability> listData=new ArrayList();
		...
		/*if(environment.equals("QA")) {
			importDeleteDate=dbModule.runQuery("select provideruri,import_deletedate from DigitalFirst_Replica..programs where provideruri in ("+queryInputAll+")"
					+ " and import_deletedate is null", inputDetails);
		}
		else {
			importDeleteDate=dbModule.runQuery("select provideruri,import_deletedate from DigitalFirst..programs where provideruri in ("+queryInputAll+")"
					+ " and import_deletedate is null", inputDetails);
		}
		...
		actual=dbModule.runQuery("select * from Mosaic..credit where provider_id=216", inputDetails);
